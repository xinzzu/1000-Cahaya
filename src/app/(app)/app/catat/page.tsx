...
export default function CatatPage() {
  ...
  return (
    // ⬇️ TIDAK pakai padding di sini—biar ikut dari layout (px-4 pt-4 pb-[88px])
    <main>

      {/* Header (match homepage spacing) */}
      <header className="text-center">
        <h1 className="text-xl font-semibold">Catat Aktivitas</h1>
        <p className="text-sm text-black/60">Hitung jejak karbon bulanan Anda</p>
      </header>

      {/* Divider primary lebar container (sama gaya homepage) */}
      <div
        className="mt-3 h-[2px] w-full"
        style={{ backgroundColor: "var(--color-primary)" }}
      />
      ...
      <div className="mt-3 space-y-3">
        {categories.map((c) => (
          <Link
            ...
            style={{ borderColor: "var(--color-primary)" }}
          >
            {/* Ukuran icon DIKUNCI supaya konsisten */}
            <div className="grid h-14 w-14 place-items-center rounded-xl bg-[color:var(--color-secondary)]">
              <Image src={c.icon} alt="" width={32} height={32} />
            </div>
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="text-sm text-black/60 truncate">{c.subtitle}</div>
            </div>

            {/* Chevron kanan → ukuran dikunci juga */}
      ...
      {/* List dengan separator yang halus, lebar pas container */}
      <div className="mt-2">
        {saved.map((s, i) => (
      ...
      {/* CTA (tinggi & lebar sama seperti di screen lain) */}
      <div className="mt-4">
        <Button size="md" className="w-full sm:h-10">
          Lihat Riwayat Jejak Karbon Anda
        </Button>
